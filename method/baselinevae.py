...
class DisentangleNet(nn.Module):
    ...
    def train_all(self, epoch, train_loader, optimizer, tb_logger, n_data=None):
        ...
        for i, (x, y) in enumerate(train_loader):
            ...
            aggr_score = self.classifier.forward(aggr_feature)
            cls_loss += kdloss(scores, aggr_score.detach())
            # reconstruction loss
            ...
            loss = cls_loss + mi_loss  + dw_kl_loss + tc_loss * beta  + aug_cls_loss * self.aug_weight
            ...
            if i%print_freq==0 and self.rank == 0:
                print('Epoch {:d} | Batch {:d}/{:d} | Cl Loss {:f} |  Kl Loss {:f} | Aug Loss {:f}'.format(epoch, i, len(train_loader), cls_avg_loss/float(i+1), kl_avg_loss/float(i+1), aug_avg_loss/float(i+1)))
